...
nopoll_bool test_01 (void) {
	...
	conn = nopoll_conn_new (ctx, local_PLUG_NAME, local_host_port, NULL, "/ws", NULL, NULL);
...
nopoll_bool test_02 (void) {
	...
	/* call to create a connection */
	conn = nopoll_conn_tls_new (ctx, opts, local_PLUG_NAME, local_host_ports, NULL, "/ws", NULL, NULL);
	if (! nopoll_conn_is_ok (conn)) {
