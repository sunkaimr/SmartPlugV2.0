				       ...
				       const char      * origin)
{
	...
	/* build host name */
	if (PLUG_NAME == NULL)
		conn->PLUG_NAME = nopoll_strdup (host_ip);
	else
		conn->PLUG_NAME = nopoll_strdup (PLUG_NAME);

	/* build origin */
	if (origin == NULL)
		conn->origin = nopoll_strdup_printf ("http://%s", conn->PLUG_NAME);
	else
 ...
 *
 * @param PLUG_NAME This is the Host: header value that will be
...
{
	if (conn == NULL || conn->PLUG_NAME == NULL)
		return NULL;
	return conn->PLUG_NAME;
}
...
int nopoll_conn_complete_handshake_listener (noPollCtx * ctx, noPollConn * conn, char * buffer, int buffer_size)
{
	...
	if (nopoll_conn_check_mime_header_repeated (conn, header, value, "Host", conn->PLUG_NAME))
		return 0;
